...
static ucli_status_t
ofconnectionmanager_ucli_ucli__async_stats__(ucli_context_t *uc)
{
    UCLI_COMMAND_INFO(uc,
                      "async_stats", 0,
                      "$summary#Show former cxns outstanding async stats.");

    ind_cxn_former_async_stats_show(&uc->pvs);

    return UCLI_STATUS_OK;
}

static ucli_status_t
ofconnectionmanager_ucli_ucli__async_stats_clear__(ucli_context_t *uc)
{
    UCLI_COMMAND_INFO(uc,
                      "async_stats_clear", 0,
                      "$summary#Clear former cxns outstanding async stats.");

    ind_cxn_former_async_stats_clear(&uc->pvs);
...
static ucli_command_handler_f ofconnectionmanager_ucli_ucli_handlers__[] =
{
    ...
    ofconnectionmanager_ucli_ucli__async_stats__,
    ofconnectionmanager_ucli_ucli__async_stats_clear__,
